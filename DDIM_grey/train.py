...
from utils.tools import train_one_epoch, load_yaml, plot_losses
import torch
...
def train(config):
    ...
    model_checkpoint = ModelCheckpoint(**config["Callback"])
        ...
        #ModelCheckpoint(**config["Callback"]))
    ...
    for epoch in range(start_epoch, config["epochs"] + 1):
        train_loss = train_one_epoch(trainer, loader, optimizer, device, epoch)
        print(f'Epoch [{epoch}], Train Loss: {train_loss:.4f}')

        train_losses.append(train_loss)

        model_checkpoint.step(train_loss, model=model.state_dict(), config=config,
                              optimizer=optimizer.state_dict(), start_epoch=epoch,
                              model_checkpoint=model_checkpoint.state_dict())

    plot_losses(train_losses, "./loss/loss_plot.png")
